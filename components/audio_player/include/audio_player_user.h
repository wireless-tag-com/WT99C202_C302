...
/**
 * @brief 音频播放结束回调函数类型定义
 */
typedef void (*audio_player_stop_cb_t)(void);
...
/**
 * @brief 设置音频播放结束回调函数，重新打开全双工模式
 *
 * @param callback 播放结束回调函数指针
 */
void audio_player_set_stop_callback(audio_player_stop_cb_t callback);

/**
 * @brief 启用MP3硬件播放器
 *
 * 启用MP3硬件播放器，用于替代软件MP3播放器
 */
void audio_player_mp3_hardware_player_enable(void);
