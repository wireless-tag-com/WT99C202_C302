...
/**
 * @brief 音乐模式状态变化回调函数类型
 * @param is_music_mode true 进入音乐模式（需关闭全双工），false 退出音乐模式（开启全双工）
 */
typedef void (*aiha_music_mode_cb_t)(bool is_music_mode);

/**
 * @brief 设置音乐模式状态变化回调
 * @param callback 回调函数指针
 * @details 当检测到音乐播放指令或音乐结束时会调用此回调
 */
void aiha_websocket_set_music_status_callback(aiha_music_mode_cb_t callback);
