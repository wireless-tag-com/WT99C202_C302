...
ignore_dir = (".git", "build", ".vscode", "output", "managed_components", "tool", "debug")
...
def generate_release(src, dst, static_link_path, ignore_dir=None):
    ...
    # 获取target信息
    sdkconfig_path = os.path.join(src, 'sdkconfig')
    target = 'unknown'
    ...
    for name, path in release_components.items():
        ...
        rm_emty_dir(path)
        # 使用带target后缀的库文件名
        lib_with_target = f'lib{name}_{target}.a'
        shutil.copy(f'{static_link_path}/{name}/lib{name}.a', f'{path}/{lib_with_target}')
